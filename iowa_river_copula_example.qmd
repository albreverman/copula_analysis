...
extract_design_events_2d <- function(Iso, prediction, N_Ensemble = 0) {
  ...
  if (N_Ensemble > 0) {
    ...
    # If there are no valid KDE weights, return an empty 0-row matrix/data frame with the same number of columns as Iso_met
    if (length(ok) == 0) {
...
kde_contour_plot <- ggplot(contourLines_kde_100, aes(marshalltown, marengo)) +
