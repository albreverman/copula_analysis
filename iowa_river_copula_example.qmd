...
combine_isolines <- function(P1, P2,
                             ...
                             thres1 = NA, thres2 = NA,
                             con1_name = "Var1", con2_name = "Var2") {
  type <- match.arg(type)

  stopifnot(is.data.frame(P1), ncol(P1) == 2, is.data.frame(P2), ncol(P2) == 2)

  # Standardize column names for internal work
  colnames(P1) <- c("x", "y")
  colnames(P2) <- c("x", "y")

  # Global ranges (as in the original code: include Data too; caller can pass that in if needed)
  x_min <- min(c(P1$x, P2$x), na.rm = TRUE)
  x_max <- max(c(P1$x, P2$x), na.rm = TRUE)
  y_min <- min(c(P1$y, P2$y), na.rm = TRUE)
  y_max <- max(c(P1$y, P2$y), na.rm = TRUE)

  step <- 10^-dec

  # --- A) envelope using x-grid: y(x) = max(y from P1 at x, y from P2 at x)
  x_grid <- round(seq(x_min, x_max, by = step), dec)

  P1x <- round(P1$x, dec)
  P2x <- round(P2$x, dec)

  y_on_x <- vapply(x_grid, function(xx) {
    y1 <- P1$y[P1x == xx]
    y2 <- P2$y[P2x == xx]
    # original code uses max(...) across available values; if none -> -Inf
    out <- max(c(y1, y2), na.rm = TRUE)
    if (is.infinite(out)) NA_real_ else out
  }, numeric(1))

  ptsA <- data.frame(x = x_grid, y = y_on_x)
  ptsA <- ptsA[!is.na(ptsA$y), , drop = FALSE]

  # --- B) envelope using y-grid: x(y) = max(x from P1 at y, x from P2 at y)
  y_grid <- round(seq(y_min, y_max, by = step), dec)

  P1y <- round(P1$y, dec)
  P2y <- round(P2$y, dec)

  x_on_y <- vapply(y_grid, function(yy) {
    x1 <- P1$x[P1y == yy]
    x2 <- P2$x[P2y == yy]
    out <- max(c(x1, x2), na.rm = TRUE)
    if (is.infinite(out)) NA_real_ else out
  }, numeric(1))

  ptsB <- data.frame(x = x_on_y, y = y_grid)
  ptsB <- ptsB[!is.na(ptsB$x), , drop = FALSE]

  # --- C) merge, round, de-duplicate by x, sort by x (matches the original spirit)
  pts <- rbind(ptsA, ptsB)
  pts$x <- round(pts$x, dec)
  pts$y <- round(pts$y, dec)

  pts <- pts[!duplicated(pts$x), , drop = FALSE]
  pts <- pts[order(pts$x), , drop = FALSE]

  # Optional endpoints (only done in the original code for Combined)
  if (isTRUE(end) && type == "Combined") {
    # original: rbind(con2 first, pts, con1 last)
    first <- P2[1, c("x", "y"), drop = FALSE]
    last  <- P1[nrow(P1), c("x", "y"), drop = FALSE]
    pts <- rbind(first, pts, last)
    pts$x <- round(pts$x, dec)
    pts$y <- round(pts$y, dec)
    pts <- pts[order(pts$x), , drop = FALSE]
  }

  # --- D) resample along arc length to exactly `interval` points
  # Normalize to [0,1] to make x/y comparable for distance calculation
  ...
  if (max(s) == 0) {
    Iso <- pts[rep(1, interval), , drop = FALSE]
  } else {
    s_out <- seq(0, max(s), length.out = interval)
    x_res <- approx(x = s, y = x01, xout = s_out, ties = "ordered")$y
    y_res <- approx(x = s, y = y01, xout = s_out, ties = "ordered")$y

    # back-transform
    Iso <- data.frame(
      x = x_res * (max(pts$x) - min(pts$x)) + min(pts$x),
      y = y_res * (max(pts$y) - min(pts$y)) + min(pts$y)
    )
  }

  # Threshold filtering (only present in original for Con1/Con2 types)
  if (type == "Con1" && !is.na(thres1)) {
    Iso <- Iso[Iso$x >= thres1, , drop = FALSE]
  }
  if (type == "Con2" && !is.na(thres2)) {
    Iso <- Iso[Iso$y >= thres2, , drop = FALSE]
  }

  # Return with original column names
  colnames(Iso) <- c(con1_name, con2_name)
  Iso
}
