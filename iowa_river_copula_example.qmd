...
# This function takes a data frame containing isoline points for one return period, rp, but multiple curves, distinguished by scenario, and produces a new curve that is the upper envelope (for each x-value, it keeps the maximum y-value across all scenarios)
...
plot(head(copula_sample, 1000))
```

```{r}

# KDE values along an isoline in *flow space* using your objects:
...
kde_along_isoline <- function(Iso,
  ...
  # --- standardize isoline matrix (eval points) ---
  Iso_mat <- Iso %>%
    dplyr::select(marshalltown, marengo) %>%
    as.data.frame()

  # --- choose KDE input points ---
  if (Source == "Sample") {
    xdat <- cop_sample %>%
      dplyr::select(Marshalltown, Marengo) %>%
      as.data.frame()

    # truncate extreme simulated values relative to isoline scale (or data scale if provided)
    ref <- if (!is.null(Data)) {
      Data %>% dplyr::select(Marshalltown, Marengo) %>% as.data.frame()
    } else {
      Iso_mat
    }
    ...
    if (is.null(Data)) stop("When Source='Data', you must supply `Data` (Marshalltown, Marengo).")
    xdat <- Data %>%
      dplyr::select(Marshalltown, Marengo) %>%
      stats::na.omit() %>%
      as.data.frame()
  ...
  # ks::kde expects matrices/data.frames with 2 columns
  pred <- ks::kde(x = xdat, eval.points = Iso_mat, binned = TRUE)$estimate

  # normalize to [0,1] for plotting/contours
  rng <- range(pred, na.rm = TRUE)
  contour <- if (is.finite(rng[1]) && is.finite(rng[2]) && rng[2] > rng[1]) {
    (pred - rng[1]) / (rng[2] - rng[1])
  } else {
...
# Pick "design events" on an isoline in your coordinate system:
...
design <- extract_design_events_2d(Iso100, kde_out$prediction, N_Ensemble = 200)
designEvent <- design$MostLikelyEvent

contourLines_kde <- envelope_curves %>%
  group_by(rp) %>%
  group_split() %>%
  map_dfr(function(df_iso){
...
data <- rbind(marshalltown_con, marengo_con) %>%
  select(Marshalltown, Marengo)

kde_contour_plot <- ggplot(contourLines_kde, aes(marshalltown, marengo)) +
  geom_path(aes(group = rp, color = kde_scaled), size = 1.2) +
  scale_color_distiller(palette = "RdYlBu", direction = 1,
                        name = "KDE (scaled)", limits = c(0, 1)) +
...
kde_contour_plot +
  geom_point(data = head(copula_sample, n = 10000),
             aes(Marshalltown, Marengo),
             inherit.aes = FALSE,
             color = "darkgrey", shape = 21) +
  geom_point(data = data,
             aes(Marshalltown, Marengo),
             color = "black", shape = 21) + 
  geom_point(data = design$MostLikelyEvent,
             aes(marshalltown, marengo),
             inherit.aes = FALSE,
             color = "black", fill = "blue",
             shape = 23, size = 2, stroke = 1.1)
```
