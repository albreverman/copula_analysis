---
title: "Iowa River Copula Example"
format: html
editor: visual
...
time_sep <- B17_days_between(da_marshalltown)
...
evplot(marshalltown_con$Marshalltown, main = "Marengo"); 
# evdistq(quagev, para = marshalltown_gev)
...
```{r eval=FALSE}
write.csv(marengo_con, file = "marengo_con.csv")
write.csv(marshalltown_con, file = "marshalltown_con.csv")
```
...
write.csv(df, "iowa_river_contours.csv", row.names = FALSE)
```
...
## User-specified Joint Return Periods

```{r}

# Return periods
rp_vec <- c(10, 50, 100, 200, 500)
```
...
```{r}
compute_contour_lines <- function(
    return_periods,
    x, y, z,
    mu,
    stop_on_missing = TRUE
) {
  purrr::map_dfr(return_periods, function(rp) {

    xy <- grDevices::contourLines(x, y, z, levels = mu * rp)

    if (length(xy) == 0) {
      msg <- paste("No contour found for RP =", rp)
      if (stop_on_missing) stop(msg) else { warning(msg); return(NULL) }
    }

    do.call(rbind, lapply(seq_along(xy), function(i) {
      data.frame(
        id                = i,
        level             = xy[[i]]$level,
        marshalltown_cdf  = xy[[i]]$x,
        marengo_cdf       = xy[[i]]$y,
        rp                = rp
      )
    }))
...
contourLines_A <- compute_contour_lines(
  return_periods  = rp_vec,
  x               = x,
  y               = y,
  z               = z,
  mu              = mean_waiting_time_A,
  stop_on_missing = TRUE   # set FALSE to skip missing RPs with a warning instead
)
```
...
contourLines_B <- compute_contour_lines(
  return_periods  = rp_vec,
  x               = x,
  y               = y,
  z               = z,
  mu              = mean_waiting_time_B,
  stop_on_missing = TRUE   # set FALSE to skip missing RPs with a warning instead
)
```
...
ggplot(contourLines, aes(marshalltown, marengo, color = scenario, linetype = factor(aep))) +
  geom_path(linewidth = 1) +
...
scale_linetype_manual(values = c(
  ...
  ylim(0, 80000) +
  theme_bw() +
  labs(color = "Scenario", linetype = "Annual Exceedance Probability") +
  xlab("Marshalltown Flow (cfs)") +
  ylab("Marengo Flow (cfs)") +
  ggtitle("") +
  coord_fixed(ratio = 1)
...
contourLinesPlot <- ggplot() +
  geom_path(
    data = contourLines,
    aes(marshalltown, marengo, color = scenario, linetype = factor(aep)),
    linewidth = 0.8, alpha = 0.7
  ) +
  geom_path(
    data = envelope_curves,
    aes(marshalltown, marengo, linetype = factor(aep)),
...
scale_linetype_manual(values = c(
  ...
  labs(
    color = "Scenario",
    linetype = "Annual Exceedance Probability",
...
## KDE and Design Event

```{r}

compute_isoline_results <- function(
    return_periods,
    envelope_curves,
    copula_sample,
    N_Ensemble = 200,
  ...
  # ── 1. Filter to requested RPs and split into a list of data frames ──────────
  iso_list <- envelope_curves %>%
    dplyr::filter(rp %in% return_periods) %>%
    dplyr::group_by(rp) %>%
    dplyr::group_split()

  # Name each element by its RP value for easy downstream access
  rp_vals       <- purrr::map_dbl(iso_list, ~ unique(.x$rp))
  names(iso_list) <- as.character(rp_vals)

  # ── 2. For each RP: KDE along isoline + most-likely design event ─────────────
  purrr::map(iso_list, function(df_iso) {

    kde_out <- kde_along_isoline(
      Iso        = df_iso,
    ...
    stopifnot(length(kde_out$prediction) == nrow(df_iso))

    design <- extract_design_events_2d(
      df_iso,
      kde_out$prediction,
    ...
    list(
      isoline      = df_iso %>%
                       dplyr::mutate(
                         kde        = kde_out$prediction,
                         kde_scaled = kde_out$contour
                       ),
      kde_out      = kde_out,
      design       = design,
      design_event = design$MostLikelyEvent
    )
  })
}

# ── Run everything in one call ─────────────────────────────────────────────────
iso_results <- compute_isoline_results(
  return_periods  = rp_vec,
  envelope_curves = envelope_curves,
  copula_sample   = copula_sample,
  N_Ensemble      = 200,
  gridsize        = c(400, 400)
)

# ── Flatten all KDE-annotated isolines into one tidy data frame ───────────────
contourLines_kde <- purrr::map_dfr(
  iso_results,
  ~ .x$isoline,
  .id = "rp"                        # adds a character column "rp"
) %>%
  dplyr::mutate(rp = as.numeric(rp),
                aep = 1/rp)

# ── Flatten all design events into one data frame ─────────────────────────────
design_events_all <- purrr::map_dfr(
  iso_results,
  ~ .x$design_event,
  .id = "rp"
) %>%
...
contourLines_kde <- contourLines_kde %>%
  dplyr::mutate(
    aep        = format(1 / rp, scientific = FALSE),
    path_group = paste(rp, id, sep = "_")            # <-- must be saved back
  )

names(contourLines_kde)
...
ggplot() +
  ...
  geom_path(data = contourLines_kde, aes(x = marshalltown, y = marengo, group = aep, color = kde_scaled), linewidth = 1.2) +
  ...
  geom_point(
    data = data,
    aes(x = Marshalltown, y = Marengo, shape = Scenario, color = Scenario),
    inherit.aes = FALSE,
    size = 2, stroke = 1
  ) +
  scale_shape_manual(
    values = c(Marshalltown_Conditioning = 1, Marengo_Conditioning = 4),
    name = "Scenario"
  ) +
  scale_color_manual(
    values = c(Marshalltown_Conditioning = "darkgreen", Marengo_Conditioning = "purple"),
    name = "Scenario"
  ) +
  geom_point(
    data = design_events_all,                                         # <-- single data frame
    aes(x = marshalltown, y = marengo, fill = "Design Event"),
    inherit.aes = FALSE,
    color = "black", shape = 23, linewidth = 2, stroke = 1.1
  ) +
  geom_label(                                                         # <-- label each diamond
    data = design_events_all,
    aes(x = marshalltown, y = marengo, label = paste0(rp, "-yr")),
    inherit.aes = FALSE,
    nudge_y = 1500, size = 2.5, label.size = NA, fill = NA
  ) +
  geomtextpath::geom_textpath(
    data = contourLines_kde,
    aes(x = marshalltown, y = marengo,
        label = aep, group = path_group),       # <-- pre-computed character column
    color          = "black",
    linetype       = 0,
    size           = 4,
    text_smoothing = 10,
  ...
  scale_fill_manual(values = c("Design Event" = "red"), name = "") +
...
x_breaks <- seq(0, 1, length.out = n_x + 1)
y_breaks <- seq(0, 1, length.out = n_y + 1)

grid_pts <- expand.grid(x = x_breaks, y = y_breaks)

ggplot() +
  # Lines placed exactly at break positions
  geom_vline(xintercept = x_breaks, color = "steelblue", linewidth = 0.3) +
  geom_hline(yintercept = y_breaks, color = "steelblue", linewidth = 0.3) +
  # Circles at every (x_break, y_break) crossing
  geom_point(
    data  = grid_pts,
    aes(x, y),
    shape = 21,
  ...
  coord_fixed(xlim = c(-0.1, 1.1), ylim = c(-0.1, 1.1), expand = FALSE) +
  theme_minimal() +
  labs(
    title = sprintf("", n_x, n_y),
    x = "x", y = "y"
  )
```
...
ggplot(contourLines_kde_100, aes(marshalltown, marengo)) +
  geom_point(
    data = data,
    aes(x = Marshalltown, y = Marengo, shape = Scenario, color = Scenario),  # color mapped as aesthetic
    inherit.aes = FALSE,
    size = 2, stroke = 1
  ) +
  scale_shape_manual(
  ...
  scale_color_manual(                                                          # new: drives legend colors
    values = c(Marshalltown_Conditioning = "darkgreen", Marengo_Conditioning = "purple"),
    name = "Scenario"                                                          # same name = merged legend
  ) +
  theme_bw() + coord_fixed(1) +
  xlim(0, 80000) + ylim(0, 80000) +
  labs(x = "Marshalltown Flow (cfs)", y = "Marengo Flow (cfs)")
