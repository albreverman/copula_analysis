...
```{r}
X <- read.csv("RG_Sg_2022.csv", header = TRUE, sep = ",", na.strings = c("-9999")) %>% mutate(Value = Surge,                                                                     Date = as.POSIXct(dstring, format = "%m/%d/%Y %H:%M")) %>% select(Date, Value)

Y <- read.csv("Rg_Pr_2022.csv", header = TRUE, sep = ",") %>% mutate(Value = pr_AORC_01hr, Date = as.POSIXct(dstring, format = "%m/%d/%Y %H:%M")) %>% select(Date, Value)
...
```{r}
X_time_series_plot <- ggplot(X, aes(x = Date, y = Value)) +
  geom_line() +
  theme_bw() +
  xlab("") +
  ylab(X_label) +
...
Y_time_series_plot <- ggplot(Y, aes(x = Date, y = Value)) +
  geom_line() +
  theme_bw() +
  xlab("") +
  ylab(Y_label) +
...
# Extract Variable X PDS

```{r}
X_clean <- X |> filter(!is.na(Date) & !is.na(Value))

# Create data frame with obs and time columns
pot_X <- data.frame(
  obs = X_clean$Value,
  time = as.numeric(X_clean$Date)  # Convert to numeric
)

# Remove any rows with NA, NaN, or Inf
pot_X <- pot_X[complete.cases(pot_X), ]
pot_X <- pot_X[is.finite(pot_X$obs) & is.finite(pot_X$time), ]

time_sep <- 3600 * 24 * 2 # 2 days in seconds
thresh_X <- 0.1 # m

# Extract partial duration series
pds_X <- POT::clust(data = pot_X,
                              u = thresh_X, # threshold
...
years_X <-
  as.numeric((
    max(X_clean$Date) - min(X_clean$Date) + 1
  ) / 365.25)
rate_X <- nrow(df_pds_X) / years_X

# Plot original time series and PDS
X_time_series_plot +
  geom_hline(yintercept = thresh_X, linetype = "dashed") +
  geom_point(data = df_pds_X, aes(x = Date, y = Value), colour = "#A60F2D", shape = 19, size = 2) 
```

# Extract Variable Y PDS

```{r}
Y_clean <- Y |> filter(!is.na(Date) & !is.na(Value))
  
# Create data frame with obs and time columns
pot_Y <- data.frame(
  obs = Y_clean$Value,
  time = as.numeric(Y_clean$Date)  # Convert to numeric
)

# Remove any rows with NA, NaN, or Inf
pot_Y <- pot_Y[complete.cases(pot_Y), ]
pot_Y <- pot_Y[is.finite(pot_Y$obs) & is.finite(pot_Y$time), ]

# Verify it's clean
anyNA(pot_Y)
any(!is.finite(as.matrix(pot_Y)))

time_sep <- 3600 * 24 * 3 # 3 days in seconds
thresh_Y <- 25 # mm

# Extract partial duration series
pds_Y <- POT::clust(data = pot_Y,
                              u = thresh_Y, # threshold
                              tim.cond = time_sep, # time separation
                              clust.max = TRUE) # extract cluster maxima
...
years_Y <-
  as.numeric((
    max(Y_clean$Date) - min(Y_clean$Date) + 1
  ) / 365.25)
rate_Y <- nrow(df_pds_Y) / years_Y

# Plot original time series and PDS
Y_time_series_plot +
  geom_hline(yintercept = thresh_Y, linetype = "dashed") +
  geom_point(data = df_pds_Y, aes(x = Date, y = Value), colour = "#A60F2D", shape = 19, size = 2) 
```
...
lag <- 3 * 24 # -3 to +3 day lags

X_con <- left_join(X_clean, df_pds_Y, by = "Date") %>%
...
result <- MultiHazard::Kendall_Lag(Data = X_con,
...
Y_con <- left_join(Y_clean, df_pds_X, by = "Date") %>%
  select(Date, Value.x, Value.y) %>%
  rename(X = Value.x,
         Y = Value.y) 
...
# Calculate Kendall's tau at different lags
result <- MultiHazard::Kendall_Lag(Data = Y_con,
                      Lags = seq(-lag, lag, 1),  # lag is applied to second named variable
                      PLOT = TRUE, # display plot
                      GAP = 0.1) # length of y-axis above and below max and min                                       
...
# Scenario A (Extremal Variable X) Marginal Distributions

Scenario A is defined by extremal events (PDS) at the X location/variable and the coincident events at the Y location/variable. The conditioning variable is X, which are conditioned on Y.
